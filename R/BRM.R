...
BRM_temp = function(Y,t.values,seed.no=1,burn=2000,nmc=2000,thin=10, varrho = 0.1, pro.var.theta.2 = 0.0002, pro.var.theta.3 = 0.05, mu = 1, rho.sq = 1){
  ...
  {
    y_temp <- function(i){
      y = as.numeric(Y[i,1:max(t.values)])
      return(y)
    }
    for (i in 1:N){

      temp.d = data.frame(y=y_temp(i), t=c(1:max(t.values)))
  ...
  for (s in 1:(S-1)){
    ...
    if (s == 1){
      ...
      f.i.vec = function(i,theta.1.i,theta.2.i,theta.3.i, xi.i){
        res = matrix(data = f(c(t.values), theta.1 = theta.1.i, theta.2 = theta.2.i, theta.3 = theta.3.i, xi = xi.i), ncol = 1)
        return(res)
      }
      ...
      h.i.vec = function(i,theta.2.i,theta.3.i,xi.i){
        res = matrix(data = h(c(t.values), theta.2 = theta.2.i, theta.3 = theta.3.i, xi = xi.i), ncol = 1)
        return(res)
      }
      ...
      H.mat = function(theta.2.vec, theta.3.vec, xi.vec){
        res = norm_vec(x = h.i.vec(i = i, theta.2.i = theta.2.vec[i], theta.3.i = theta.3.vec[i], xi.i = xi.vec[i]))^2
        return(res)
      }
      ...
      alpha.ft.2 = function(i = i, theta.2.i, old.theta.2.i, theta.1.i, theta.3.i, xi.i, sigma.sq, alpha.2, sigma.2.sq){
      ...
      alpha.ft.3 = function(i = i, theta.3.i, old.theta.3.i, theta.1.i, theta.2.i, xi.i, sigma.sq, alpha.3, sigma.3.sq){
    ...
    {
      temp.1 = N*length(t.values)/2 + varrho
      temp.2 = c()
      for(i in 1:N){
...
#' Function that implement Bayesian Richards model without covariates for individual subjects (Note: because BRM is not hierarhical model, it is recommended to use the diffuse prior for the error vaiance => varrho = 0.1)
#'
